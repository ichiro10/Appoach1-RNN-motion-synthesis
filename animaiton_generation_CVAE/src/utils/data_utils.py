...
def load_data(path_to_dataset, subjects, actions):
	...
	for subj in subjects:
		for action_idx, action in enumerate(actions):
			...
			logging.info("Reading subject {0}, action {1}".format(subj, action))
			#filename = '{0}/S{1}/{2}_{3}.txt'.format( path_to_dataset, subj, action, subact)
			#dir = os.path.dirname(__file__)
			action_folder = os.path.join(path_to_dataset, subj, action)
			for file_name in os.listdir(action_folder):
					file = os.path.join(action_folder, file_name)
					action_sequence = readCSVasFloat(file)
					...
					trainData[(subj, action, 'even')] = the_sequence
...
def normalize_data( data, data_mean, data_std, dim_to_use, actions):
	...
	for key in data.keys():
		data_out[ key ] = np.divide( (data[key][:, 0:72] - data_mean), data_std )
...
def define_actions( action ):
	...
	if action == "all":
		return actions
	if action == "all_srnn":
		return ["hiding", "showing", "showingphone", "stopping",  "waving"]
...
def read_all_data( actions, seq_length_in, seq_length_out, data_dir):
	...
	train_subject = ['ES','Normal']
	test_subject = ['Neurotic']
	print(data_dir)
	train_set, complete_train = load_data(data_dir,train_subject,actions)
	test_set,  complete_test  = load_data(data_dir,test_subject, actions)
