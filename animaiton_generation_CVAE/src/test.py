...
from models.motionpredictor import MotionPredictor_CVAE
...
parser.add_argument('--data_dir', dest='data_dir', help='Data directory',
					default=os.path.normpath("c:\Users\aghammaz\Desktop\GIT\DeepL_VirtualCharactersMotions\Style_data"), type=str)
...
def get_srnn_gts( actions, model, test_set, data_mean, data_std, dim_to_ignore, to_euler=True ):
    ...
    for action in actions:
        ...
        _, _, srnn_expmap = model.get_batch_srnn( data = test_set, action = action, device = device)
...
def main():
    ...
    f = os.path.join(dir,train_dir , "model_" + str(args.load_model))
